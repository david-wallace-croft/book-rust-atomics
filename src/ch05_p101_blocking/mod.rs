...
impl<T> Receiver<'_, T> {
  ...
  pub fn receive(self) -> T {
    while !self.channel.ready.swap(false, Acquire) {
      info!("Consumer thread is parking consumer thread");

      thread::park();

      info!("Consumer thread has unparked");
    }
...
impl<T> Sender<'_, T> {
  pub fn send(
    ...
    info!("Producer thread is unparking consumer thread");

    self.receiving_thread.unpark();

    info!("Producer thread has unparked consumer thread");
  }
}
...
mod test {
  ...
  fn test1() {
    ...
    thread::scope(|s: &Scope<'_, '_>| {
      ...
      info!("Consumer thread is receiving message");
